...
if __name__ == "__main__":
    ...
    if keysize == 256: num_rounds = 14
    round_keys = [None for i in range(num_rounds+1)]
    round_keys_int = [None for i in range(num_rounds+1)]
    for i in range(num_rounds+1):
        round_keys[i] = (key_words[i*4] + key_words[i*4+1] + key_words[i*4+2] + key_words[i*4+3]).get_bitvector_in_hex()
        round_keys_int[i] = (key_words[i*4] + key_words[i*4+1] + key_words[i*4+2] + key_words[i*4+3])
    print("\n\nRound keys in hex (first key for input block):\n")
    for round_key in round_keys:
        print(round_key)'''
...
def get_round_keys(keysize, key_text):
    """
    Entry point for other modules. 
    Returns a list of BitVector objects, each representing a round key.
    """
    # 1. Handle key length (padding or clipping)
    key_text = key_text.strip()
    needed_bytes = keysize // 8
    if len(key_text) < needed_bytes:
        key_text += "0" * (needed_bytes - len(key_text))
    else:
        key_text = key_text[:needed_bytes]
    
    key_bv = BitVector(textstring=key_text)

    # 2. Generate key words based on size
    if keysize == 128:
        key_words = gen_key_schedule_128(key_bv)
        num_rounds = 10
    elif keysize == 192:
        ...
        raise ValueError("Invalid keysize. Must be 128, 192, or 256.")

    # 3. Concatenate words into round keys
    round_keys = []
    for i in range(num_rounds + 1):
        # Each round key is 4 words (128 bits)
        r_key = key_words[i*4] + key_words[i*4+1] + key_words[i*4+2] + key_words[i*4+3]
...
if __name__ == "__main__":
    # Standard interactive behavior for testing
    size = int(input("\nAES Key size (128, 192, 256): "))
    user_key = input("\nEnter key: ")
    
    # Call the new function
    all_round_keys = get_round_keys(size, user_key)
    
    print("\nRound keys in hex:")
    for i, rk in enumerate(all_round_keys):
        print(f"Round {i}: {rk.get_bitvector_in_hex()}")
